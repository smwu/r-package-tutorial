...
citation:
  type: webpage
  title: "Building Your Own R Package: A Tutorial for the UCL R User Group"
  author:
    - family: Wu
      given: Stephanie
  issued: 2025-11-27
  url: https://smwu.github.io/r-package-tutorial/tutorial.html 
...
```{r examples}
# Example codes to reformat
example_postcodes <- c("wc1e 6bt", ".NW12DB", "EH12NG", "SP4 7DE")
example_postcodes

# Apply the function to reformat valid postcodes
...
# Apply the function to set invalid codes to NA
...
├─ NAMESPACE          # Manages which functions are exported/imported (auto-generated by roxygen2) 
├─ postcoder.Rproj    # R project for the package
...
Every R package must specify a license, so we'll add a `LICENSE` file. Here we use the `usethis::use_mit_license()` function to add the MIT license, which is common for permissive sharing. Other options include `use_cc0_license()` for public domain release. Replace my name (Stephanie Wu) with your own name as the author of the package.
...
-   **`Version`** → Package version.
    -   When choosing a version number, anything that is still in development, will typically look like `0.0.0.9000`. Otherwise, fully functional packages ready for release follow a `major.minor.patch` output (e.g., 1.0.0), where incrementing the `patch` digit occurs for bug fixes, incrementing the `minor` digit occurs for new features or functions, and incrementing the `major` digit occurs if there is a significant restructuring to the package that would break previous code.
-   **`Authors@R`** → Structured author information using `person("First","Last", role = c("aut","cre"), email = "you@example.com")`.
    -   Possible options for roles include author ("aut"), maintainer ("cre"), contributor ("ctb"), and/or copyright holder ("cph").
...
```{verbatim description, eval=FALSE}
Package: postcoder
...
You may also want to publish your package in a journal. Some peer-reviewed, open-access journal options include:

-   **Journal of Open Source Software**: Typically quite fast turnaround
-   **The R Journal**: The official R journal focusing on R packages and programming
-   **Journal of Statistical Software**: Focuses on statistical algorithms and computing

------------------------------------------------------------------------

## 3) Add‑ons: tests, vignettes, package data, citation, GitHub

These upgrades are independent—add them when you need them.

### 3.1 Tests

Adding unit tests to your package is a great way to make sure that your functions are running properly. These tests allow you to describe what a function is expected to do, including catching errors, warnings, and messages. These tests will be run automatically every time you run `check()`. The [testing chapter](https://r-pkgs.org/testing-basics.html) in the [R Packages](https://r-pkgs.org/) tutorial provides advice and examples for creating a test suite.
...
Then, create a test file for the package function(s) (e.g., `uk_postcode_reformat()`) using the function `usethis::use_test()`. This will create a file in the `tests/testthat/` subfolder titled `test-uk_postcode_reformat.R` and initialize it with a toy example.
...
```{r data, eval=FALSE}
# e.g., 6-row demo of messy and invalid postcodes 
example_data <- data.frame(example_data = c("wc1e 6bt", ".NW12DB", "EH12NG","hiiii", "230aaa", "INVALID"))
usethis::use_data(example_data, overwrite = TRUE)
```

This will also prompt you to document your data, following the guidelines at <https://r-pkgs.org/data.html>.

To document our data, we create an `example_data.R` file within the `R/` folder and add the following information:
...
Note: in the below example vignette file, please replace all instances of === with ``` when creating the actual vignette .Rmd file.
...
Start simple, keep examples fast, and add features (tests, data, vignettes, CRAN) as your needs grow. Small, well‑scoped packages are easier to maintain and much easier for collaborators to use.
...
-   [ ] Add data with `usethis::use_data()`, remembering to document the data.
